...
def firestore_login(request, user):
    ...
    if user and user.is_authenticated and user.is_active:
        ...
        request.user = user
        logger.info(
            f"User {user.username} logged in, session saved with ID: {user.doc_id}"
        )
        return True
    logger.warning(
        f"Failed to log in user: authenticated={getattr(user, 'is_authenticated', False)}, active={getattr(user, 'is_active', False)}"
    )
    return False
