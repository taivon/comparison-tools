        ...
        if cls._instance is None:
            ...
            try:
                ...
                if os.environ.get('GAE_ENV', '').startswith('standard'):
                    ...
                    cls._instance.db = firestore.Client(credentials=credentials, project=project, database='apartments-db')
                
                # Test connection with a simple operation
                try:
                    # Try to list collections to verify database is accessible
                    list(cls._instance.db.collections(limit=1))
                    logger.info("Firestore client initialized and tested successfully")
                except Exception as test_e:
                    logger.warning(f"Firestore client initialized but database may not be ready: {test_e}")
...
class FirestoreService:
    ...
    def get_user_apartments(self, user_id):
        ...
        docs = self.db.collection('apartments').where('user_id', '==', str(user_id)).order_by('created_at', direction=firestore.Query.DESCENDING).stream()
        apartments = []
        for doc in docs:
    ...
    def get_user_preferences(self, user_id):
        ...
        docs = list(self.db.collection('user_preferences').where('user_id', '==', str(user_id)).limit(1).stream())
    ...
    def update_user_preferences(self, user_id, preferences_data):
        ...
        if docs:
            doc = docs[0]
            self.db.collection('user_preferences').document(doc.id).update(preferences_data)
