...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('DRY RUN MODE - No apartments will be updated'))
        ...
        for sqlite_user in sqlite_users:
            firestore_user = firestore_service.get_user_by_username(sqlite_user.username)
            if firestore_user:
                id_mapping[str(sqlite_user.id)] = firestore_user.doc_id
                self.stdout.write(f'  SQLite ID {sqlite_user.id} ({sqlite_user.username}) -> Firestore ID {firestore_user.doc_id}')
            else:
                self.stdout.write(
                    self.style.WARNING(f'  SQLite ID {sqlite_user.id} ({sqlite_user.username}) -> NOT FOUND in Firestore')
                )
            ...
            if old_user_id in id_mapping:
                ...
                if dry_run:
                    ...
                    try:
                        ...
                        firestore_service.update_apartment(apartment.doc_id, update_data)
                ...
                self.stdout.write(
                    self.style.WARNING(f'SKIPPING: {apartment.name} - Unknown user ID {old_user_id}')
                )
                skipped_count += 1
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(f'DRY RUN COMPLETE: Would update {len(apartments) - skipped_count} apartments')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f'MIGRATION COMPLETE: {updated_count} apartments updated, {skipped_count} skipped')
            )