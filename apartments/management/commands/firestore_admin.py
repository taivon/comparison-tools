...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--id", help="ID of specific document to view/delete")
        parser.add_argument(
            ...
            try:
                ...
                for user in users:
                    ...
                    if prefs:
                        ...
                        self.stdout.write(
                            f"  Discount Calculation: {prefs.discount_calculation}"
                        )
                        self.stdout.write("")
                if not users:
        ...
        try:
            if data_type == "apartments":
                        ...
                        json.dumps(
                            {
                                ...
                                "created_at": (
                                    apartment.created_at.isoformat()
                ...
                if prefs:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"Preferences for user {doc_id} not found")
                    )
        except Exception as e:
    ...
    def delete_document(self, service, data_type, doc_id, confirm):
        ...
        if not doc_id:
            self.stdout.write(
                self.style.ERROR("Document ID required for delete action")
            )
            return
        ...
        try:
            if data_type == "apartments":
                ...
                if success:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"Failed to delete apartment {doc_id}")
                    )
                ...
                if success:
                    self.stdout.write(
                        self.style.SUCCESS(f"Deleted preferences for user {doc_id}")
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            f"Failed to delete preferences for user {doc_id}"
                        )
                    )
        except Exception as e:
    ...
    def clear_collection(self, service, data_type, confirm):
        ...
        if not confirm:
            self.stdout.write(
                self.style.WARNING("Use --confirm to confirm clearing collections")
            )
            return
            ...
            if data_type in ["preferences", "all"]:
                ...
                for user in users:
                    if service.delete_user_preferences(user.pk):
                        count += 1
                self.stdout.write(
                    self.style.SUCCESS(f"Cleared {count} user preferences")
                )
