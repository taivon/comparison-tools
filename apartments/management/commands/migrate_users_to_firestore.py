...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for sqlite_user in sqlite_users:
            ...
            if existing_user:
                self.stdout.write(
                    self.style.WARNING(f'SKIPPING: User {sqlite_user.username} already exists in Firestore')
                )
                skipped_count += 1
                continue
            ...
            if dry_run:
                ...
                try:
                    ...
                    firestore_user = firestore_service.create_user(user_data, default_password)
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(f'DRY RUN COMPLETE: Would migrate {sqlite_users.count() - skipped_count} users')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f'MIGRATION COMPLETE: {migrated_count} users migrated, {skipped_count} skipped')
            )
            self.stdout.write(
                self.style.WARNING('IMPORTANT: All migrated users have the temporary password "temppassword123"')
            )
            self.stdout.write(
                self.style.WARNING('Users will need to reset their passwords on first login')
            )