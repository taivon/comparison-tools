...
def migrate_data():
    ...
    for apartment in apartments:
        try:
            ...
            print(f"  ✓ Migrated apartment: {apartment.name} (ID: {firestore_apartment.doc_id})")
            apartment_count += 1
    ...
    for pref in preferences:
        try:
            ...
            firestore_prefs = firestore_service.create_user_preferences(preferences_data)
            print(f"  ✓ Migrated preferences for user: {pref.user.username} (ID: {firestore_prefs.doc_id})")
            preferences_count += 1
            
        except Exception as e:
            print(f"  ✗ Failed to migrate preferences for user {pref.user.username}: {str(e)}")
...
def verify_migration():
    ...
    for user in users:
        ...
        if sqlite_apartments == firestore_count:
            ...
            print(f"  ✗ User {user.username}: SQLite={sqlite_apartments}, Firestore={firestore_count}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"Found {apartment_count} apartments and {preferences_count} user preferences to migrate")
